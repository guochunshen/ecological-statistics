...
knitr::kable(summary(pca_result), caption = "主成分分析结果")
biplot(pca_result)
```
...
knitr::kable(summary(model1), caption = "叶片经济型谱关系线性回归结果")

model2 <- lm(叶片寿命 ~ 比叶面积, data = trait_data)
knitr::kable(summary(model2), caption = "叶片寿命与比叶面积关系线性回归结果")
...
knitr::kable(summary(pca_result), caption = "群落相似性PCA分析结果")
...
knitr::kable((beta_div), caption = "群落相似性Beta多样性分析结果")
```
