...
```{r kendall-figure, fig.cap="鸟类迁徙时间与气温变化的关系散点图，显示对异常值的稳健性", echo=FALSE}
...
```{r distance-correlation-diagram, fig.cap="距离相关强弱对比示意图：左图显示弱距离相关（变量间距离模式不同步），右图显示强距离相关（变量间距离模式高度同步）", echo=FALSE}
library(ggplot2)
library(patchwork)
...
```{r non-linear-relationship, fig.cap="植物功能性状间的非线性关系散点图，显示U型关系", echo=FALSE}
# 可视化非线性关系
...
```{r mutinformation-figure, fig.cap="环境因子与物种分布的关系逻辑回归曲线", echo=FALSE}
# 可视化物种出现与环境因子的关系
par(mfrow = c(1, 2))
...
```{r time-autocorrelation-examples, fig.cap="生态学中不同强度时间自相关的实例对比：左图显示强正自相关（多年生植物种群动态），中图显示弱自相关（随机环境波动），右图显示负自相关（捕食-被捕食系统振荡）", echo=FALSE}
library(ggplot2)
library(patchwork)
...
```{r acf-patterns-figure, fig.cap="四种时间自相关模式的自相关函数对比：左上显示强正自相关的缓慢衰减模式，右上显示弱自相关的快速衰减模式，左下显示捕食者种群的负自相关振荡模式，右下显示被捕食者种群的负自相关振荡模式", echo=FALSE}
# 创建ACF图形
...
```{r acf-pacf-comparison, fig.cap="森林年轮宽度的自相关函数与偏自相关函数对比", echo=FALSE}
# 比较ACF和PACF
par(mfrow = c(1, 2))
...
```{r soil-ph-sampling, fig.cap="土壤pH值的空间取样分布图", echo=FALSE}
# 变异函数示例：土壤pH值的空间分布
set.seed(123)
...
```{r forest-spatial-distribution, fig.cap="森林树木的空间分布和胸径变异", echo=FALSE}
# 可视化空间分布和自相关
par(mfrow = c(1, 2))
...
```{r moran-monte-carlo, fig.cap="Moran's I的蒙特卡洛检验结果", echo=FALSE}
# 可视化蒙特卡洛检验结果
hist(moran_mc$res,
...
```{r lisa-analysis, fig.cap="鸟类物种丰富度的LISA分析结果", echo=FALSE}
# 可视化LISA分析结果
# LISA类型分布
...
```{r gi-analysis, fig.cap="鸟类物种丰富度的Getis-Ord Gi*分析结果", echo=FALSE}
# Gi*热点分析
...
```{r multiple-comparison, fig.cap="多重比较校正前后的p值分布对比", echo=FALSE}
# 多重比较校正
# 使用FDR校正p值
...
```{r detrended-spatial-plot, fig.cap="去趋势后的森林生物量空间分布，趋势成分已被去除", echo=FALSE}
# 去趋势后空间过程可视化
if (AIC(trend_linear) < AIC(lm(biomass ~ 1, data = spatial_df))) {
