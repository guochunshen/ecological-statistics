...
```{r pearson-figure, fig.cap="树木胸径与树高的关系散点图，显示线性相关关系。图中蓝色实心圆点表示观测数据，红色实线表示线性回归拟合线，通过颜色和点型的组合确保在彩色显示和黑白打印时都能清晰区分数据点和趋势线"}
# 可视化散点图
plot(dbh, height,
  pch = 19, col = "blue",
...
```{r spearman-figure, fig.cap="河流水质与底栖动物多样性的关系散点图，显示单调非线性关系。图中深绿色三角形表示观测数据，红色虚线表示局部加权回归拟合线"}
# 可视化关系
...
```{r distance-correlation-diagram, fig.cap="距离相关强弱对比示意图：左图显示弱距离相关（红色圆形散点，变量间距离模式不同步），右图显示强距离相关（蓝色方形散点+虚线趋势线，变量间距离模式高度同步）", echo=FALSE}
library(ggplot2)
library(patchwork)
...
```{r time-autocorrelation-examples, fig.cap="生态学中不同强度时间自相关的实例对比：左图显示强正自相关（蓝色实线表示多年生植物种群动态），中图显示弱自相关（紫色实线表示随机环境波动），右图显示负自相关（橙色实线表示捕食者种群，红色虚线表示被捕食者种群）", echo=FALSE}
library(ggplot2)
library(patchwork)
...
p3 <- ggplot() +
  geom_line(data = data.frame(Year = years, Population = predator),
            aes(x = Year, y = Population), color = "#F18F01", linewidth = 1.2, linetype = "solid") +
  geom_line(data = data.frame(Year = years, Population = prey),
            aes(x = Year, y = Population), color = "#C73E1D", linewidth = 1.2, linetype = "dashed") +
...
```{r acf-patterns-figure, fig.cap="四种时间自相关模式的自相关函数对比：左上蓝色柱状图显示强正自相关的缓慢衰减模式，右上紫色柱状图显示弱自相关的快速衰减模式，左下橙色柱状图显示捕食者种群的负自相关振荡模式，右下红色柱状图显示被捕食者种群的负自相关振荡模式", echo=FALSE}
# 创建ACF图形
...
```{r, echo=FALSE}
# 输出关键统计信息
cat("四种时间序列的自相关函数（滞后1）结果：\n",
    "强正自相关序列：", round(acf_strong$acf[2], 3), "\n",
    "弱自相关序列：", round(acf_weak$acf[2], 3), "\n",
    "捕食者序列：", round(acf_predator$acf[2], 3), "\n",
    "被捕食者序列：", round(acf_prey$acf[2], 3), "\n",
    sep = "")
```
...
1. 计算并可视化偏自相关函数（见\@ref(fig:pacf-example)）
2. 比较ACF和PACF的差异（见\@ref(fig:acf-pacf-comparison)）
...
save(years, tree_ring_width, file = "data/tree_ring_width.RData")
```
...
```{r acf-pacf-comparison, fig.cap="森林年轮宽度的自相关函数与偏自相关函数对比。左图显示自相关函数，右图显示偏自相关函数", echo=FALSE}
# 比较ACF和PACF
par(mfrow = c(1, 2))
...
```{r time-series-stationarity-comparison, fig.cap="时间序列平稳性对比示意图：左上蓝色线条表示平稳时间序列（恒定统计特性），右上紫色线条表示非平稳时间序列（具有趋势和季节性变化），左下蓝色柱状图显示平稳序列的自相关函数，右下紫色柱状图显示非平稳序列的自相关函数", echo=FALSE}
library(ggplot2)
library(patchwork)
...
```{r bird-population-ts, fig.cap="鸟类种群数量的时间序列图，展示了具有趋势、季节性和随机成分的非平稳时间序列", echo=FALSE}
...
```{r spatial-patterns, fig.cap="空间自相关模式的直观展示：有空间自相关（左上）、无空间自相关（右上）、强空间自相关（左下）、弱空间自相关（右下）", echo=FALSE}
# 绘制四种空间自相关模式的对比图
...
```{r spatial-heatmaps, fig.cap="空间自相关的热力图展示：通过插值方法生成的热力图能够更清晰地显示空间格局", echo=FALSE}
# 创建热力图展示空间自相关
...
```{r variogram-parameters-explanation, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="变异函数三个关键参数的技术解释：块金值（nugget）、基台值（sill）和变程（range）的直观展示。图中蓝色实线表示变异函数曲线，红色虚线标记块金值，绿色虚线标记基台值，橙色虚线标记变程"}
# 变异函数参数的技术解释
library(ggplot2)
...
```{r variogram-parameters-comparison, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="不同变异函数参数组合的对比：展示块金值、基台值和变程对空间依赖结构的影响。图中使用不同颜色和线型的曲线区分四种参数组合：典型情况、高测量误差、大空间变异和小尺度依赖"}
# 对比不同参数组合的变异函数
library(ggplot2)
...
```{r soil-ph-sampling, fig.cap="土壤pH值的空间取样分布图。图中使用从红色到蓝色的颜色渐变表示pH值大小，同时通过点的大小变化增强视觉区分", echo=FALSE}
# 变异函数示例：土壤pH值的空间分布
set.seed(123)
...
ggplot(spatial_df, aes(x = x, y = y, color = pH, size = pH)) +
  geom_point(shape = 16) +
...
```{r soil-ph-variogram, fig.cap="土壤pH值的经验变异函数和空间分布图", echo=FALSE}
# 计算经验变异函数
...
save(tree_coords, tree_dbh, file="data/forest_trees.RData")
```

```{r forest-spatial-distribution, fig.cap="森林树木的空间分布和胸径变异。左图显示树木空间分布，右图使用从蓝色到红色的颜色渐变表示胸径大小，同时通过圆点和三角形的点型区分小胸径和大胸径", echo=FALSE}
# 可视化空间分布和自相关
par(mfrow = c(1, 2))
...
# 根据胸径大小设置点型
dbh_pch <- ifelse(tree_dbh > median(tree_dbh), 17, 16)  # 大胸径用三角形，小胸径用圆点

plot(tree_coords,
  pch = dbh_pch, col = dbh_colors[dbh_col_idx],
  main = "树木胸径的空间分布",
  xlab = "X坐标", ylab = "Y坐标"
)
...
```{r moran-monte-carlo, fig.cap="Moran's I的蒙特卡洛检验结果。图中浅蓝色直方图表示随机化分布，红色垂直线标记观测到的Moran's I统计量", echo=FALSE}
# 可视化蒙特卡洛检验结果
hist(moran_mc$res,
...
进一步的空间自相关分析为我们提供了量化这种空间模式的统计证据。Moran's *I*统计量显著大于其期望值，这一结果表明树木胸径在空间上存在正的空间自相关，即相似大小的树木倾向于在空间上聚集分布。Geary's C统计量小于1的发现进一步支持了这一结论，因为Geary's C对局部空间差异更为敏感，其值小于1同样表明空间单元之间存在正的空间自相关。这两种空间自相关指数的相互印证，增强了我们对空间模式判断的可靠性。

为了验证这种空间自相关模式的统计显著性，我们进行了蒙特卡洛模拟检验（图\@ref(fig:moran-monte-carlo)）。检验结果显示观测到的Moran's *I*统计量在随机分布假设下的概率极低，这表明我们观测到的空间自相关模式不太可能是随机产生的，而是具有统计显著性的真实空间模式。这种显著性检验为我们的生态学解释提供了坚实的统计基础。
...
save(park_coords, bird_richness, file="data/city_parks.RData")
```
...
```{r lisa-analysis, fig.cap="鸟类物种丰富度的LISA分析结果。图中使用不同颜色和点型区分五种LISA类型：高-高（红色圆点）、低-低（蓝色三角形）、高-低（粉色方形）、低-高（浅蓝色菱形）和不显著（灰色空心圆点）", echo=FALSE}
# 可视化LISA分析结果
# LISA类型分布
ggplot(lisa_results, aes(x = x, y = y, color = lisa_type, shape = lisa_type, size = richness)) +
...
```{r gi-analysis, fig.cap="鸟类物种丰富度的Getis-Ord Gi*分析结果。图中使用不同颜色和点型区分三种类型：热点（红色圆点）、冷点（蓝色三角形）和不显著（灰色空心圆点）", echo=FALSE}
# Gi*热点分析
ggplot(lisa_results, aes(x = x, y = y, color = gi_type, shape = gi_type, size = abs(gi_star))) +
  geom_point(alpha = 0.7) +
  scale_color_manual(values = c(
    "热点" = "red", "冷点" = "blue",
    "不显著" = "gray"
...
```{r multiple-comparison, fig.cap="多重比较校正前后的p值分布对比。左图使用浅蓝色斜线填充表示原始p值分布，右图使用浅绿色反斜线填充表示FDR校正后p值分布，两图中红色虚线均标记显著性阈值0.05", echo=FALSE}
# 多重比较校正
# 使用FDR校正p值
...
hist(lisa_results$p_value,
  breaks = 20, col = "lightblue", density = 20, angle = 45,
  main = "原始p值分布", xlab = "p值"
)
...
hist(lisa_results$p_fdr,
  breaks = 20, col = "lightgreen", density = 20, angle = -45,
...
```{r spatial-stationarity-comparison, fig.cap="空间平稳性对比示意图：左上显示平稳空间过程（恒定统计特性），右上显示非平稳空间过程（具有趋势和空间异质性），左下蓝色散点和曲线表示平稳过程的变异函数，右下紫色散点和曲线表示非平稳过程的变异函数", echo=FALSE}
library(ggplot2)
library(patchwork)
...
```{r forest-biomass-spatial, fig.cap="森林生物量的空间分布图，展示了具有趋势和空间异质性的非平稳空间过程", echo=FALSE}
...
```{r trend-linear-anova, echo=FALSE}
# 进行趋势面检验
knitr::kable(anova(trend_linear), caption = "一阶趋势面F检验结果", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```

```{r trend-quadratic-anova, echo=FALSE}
knitr::kable(anova(trend_quadratic), caption = "二阶趋势面F检验结果", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```
...
```{r, echo=FALSE}
cat("残差统计：\n",
    "原始数据方差：", round(var(spatial_df$biomass), 2), "\n",
    "一阶趋势面残差方差：", round(var(residuals_linear), 2), "\n",
    "二阶趋势面残差方差：", round(var(residuals_quadratic), 2), "\n",
    sep = "")
```
...
if (AIC(trend_linear) < AIC(lm(biomass ~ 1, data = spatial_df))) {
  ...
  # 检验去趋势后过程的平稳性
  cat("去趋势后过程的统计特性：\n",
      "均值：", round(mean(detrended_biomass), 3), "\n",
      "方差：", round(var(detrended_biomass), 3), "\n",
      sep = "")
}
```
...
```{r original-spatial-plot, fig.cap="原始森林生物量空间分布，显示明显的空间趋势和异质性", echo=FALSE}
# 原始空间过程可视化
if (AIC(trend_linear) < AIC(lm(biomass ~ 1, data = spatial_df))) {
  ggplot(spatial_df, aes(x = x, y = y, color = biomass)) +
    geom_point(size = 3) +
...
```{r detrended-spatial-plot, fig.cap="去趋势后的森林生物量空间分布，趋势成分已被去除。图中使用从蓝色到红色的颜色渐变表示残差大小，同时通过圆点和三角形的点型区分负残差和正残差", echo=FALSE}
# 去趋势后空间过程可视化
if (AIC(trend_linear) < AIC(lm(biomass ~ 1, data = spatial_df))) {
  ...
  ggplot(detrended_df, aes(x = x, y = y, color = residual, shape = residual > 0)) +
    geom_point(size = 3) +
    scale_color_gradient2(low = "blue", mid = "white", high = "red") +
    scale_shape_manual(values = c(16, 17)) +  # 负残差用圆点，正残差用三角形
    labs(
      title = "去趋势后空间过程",
...
```{r original-variogram, fig.cap="原始空间过程的变异函数，显示持续上升的非平稳特征", echo=FALSE}
# 原始空间过程变异函数
if (AIC(trend_linear) < AIC(lm(biomass ~ 1, data = spatial_df))) {
...
```{r detrended-variogram, fig.cap="去趋势后空间过程的变异函数，显示收敛的平稳特征", echo=FALSE}
# 去趋势后空间过程变异函数
if (AIC(trend_linear) < AIC(lm(biomass ~ 1, data = spatial_df))) {
...
```{r spatial-decomposition, fig.cap="空间过程分解结果，展示趋势成分和随机成分的分离", echo=FALSE}
# 空间过程分解可视化
if (AIC(trend_linear) < AIC(lm(biomass ~ 1, data = spatial_df))) {
  # 创建趋势面预测
...
```{r phylogenetic-signal-figure, fig.cap="植物叶片性状的系统发育信号分析和性状距离关系", echo=FALSE}
# 加载所需包
...
```{r signal-comparison-table, echo=FALSE}
# 比较不同系统发育信号度量方法
...
knitr::kable(signal_comparison, caption = "不同系统发育信号度量方法的比较", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```
...
save(phy_tree, trait_data, file = "data/trait_data.RData")
```
...
```{r pic-comparison-figure, fig.cap="植物叶片性状的系统发育独立对比分析和性状关系", echo=FALSE}
# 可视化对比结果
par(mfrow = c(2, 2))
...
```{r phylogenetic-distribution-figure, fig.cap="植物叶片性状的系统发育分布", echo=FALSE}
par(mfrow = c(1, 1))
...
save(phy_tree, comm, file = "data/phy_comm.Rdata")
```
...
**生态学意义**：通过分析系统发育聚集与分散，研究者能够推断群落形成的主导机制，理解生态位分化与共存模式，为生物多样性维持和生态系统功能研究提供重要的进化视角。同样的分析思路可延伸至功能性状数据，通过揭示性状的聚集与发散，直接验证驱动群落组装的生态过程。
...
```{r trait-correlation-pca, fig.cap="功能性状相关性矩阵图和主成分分析双标图", echo=FALSE}
# 创建示例功能性状数据
trait_data <- data.frame(
...
knitr::kable(summary(model1)$coefficients, caption = "叶片经济型谱关系线性回归结果", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))

model2 <- lm(叶片寿命 ~ 比叶面积, data = trait_data)
knitr::kable(summary(model2)$coefficients, caption = "叶片寿命与比叶面积关系线性回归结果", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
...
```{r species-association-network, fig.cap="种间关联网络图"}
