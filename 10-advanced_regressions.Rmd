...
```{r setup-lake-data, echo=FALSE, fig.cap="浮游植物生物量与环境因子关系的可加性模型分析 - 数据准备"}
# 加载必要的包
library(mgcv)
library(ggplot2)
...
lake_data <- data.frame(
  temperature = runif(n, 5, 30),    # 水温(°C)，范围5-30°C
  nutrients = runif(n, 0.1, 5),     # 营养盐浓度(mg/L)，范围0.1-5 mg/L
  light = runif(n, 100, 2000)       # 光照强度(μmol/m²/s)，范围100-2000 μmol/m²/s
)
...
lake_data$chlorophyll <- (
  2 +  # 基准叶绿素浓度
    0.1 * (lake_data$temperature - 15)^2 - 0.02 * (lake_data$temperature - 15)^3 +  # 温度单峰效应
    2 * (1 - exp(-0.8 * lake_data$nutrients)) +  # 营养盐饱和效应
    0.001 * lake_data$light +  # 光照线性效应
    rnorm(n, 0, 0.5)  # 随机误差项
)
```

```{r build-phyto-am, fig.cap="浮游植物生物量与环境因子关系的可加性模型构建"}
# 构建可加性模型
# 使用s()函数为每个环境因子创建平滑项
# gam()函数自动选择最优平滑复杂度
...
```{r visualize-phyto-am, fig.cap="浮游植物生物量与环境因子关系的可加性模型平滑效应图"}
# 可视化各变量的平滑效应
# pages = 1 将所有图显示在同一页面
...
```{r diagnose-phyto-am, fig.cap="浮游植物生物量可加性模型诊断图"}
# 模型诊断
# 检查平滑复杂度选择和残差模式
gam.check(phyto_am)
```

```{r compare-phyto-models, fig.cap="可加性模型与线性模型比较"}
# 与线性模型比较
# 构建传统线性模型作为基准
...
```{r visualize-prediction-intervals, fig.cap="温度对浮游植物生物量影响的预测曲线（含95%置信区间）"}
# 可视化预测区间
# 加载ggplot2包（如果尚未加载）
library(ggplot2)
...
```{r, fig.cap="土壤湿度对植物存活率影响的预测曲线（含95%置信区间）"}
...
```{r, fig.cap="温度对物种出现概率影响的预测曲线（含95%置信区间）"}
# 预测新数据
new_data <- data.frame(
...
```{r, fig.cap="鸟类物种丰富度与环境因子关系的广义可加模型分析"}
# 加载必要的包
library(mgcv)
library(ggplot2)
