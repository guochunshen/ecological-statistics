...
```{r model-complexity-data, fig.cap="林小雨的森林土壤研究：模型复杂度与拟合优度的平衡。比较线性、二次、三次和10次多项式模型对植物生物量与土壤养分关系的拟合效果"}
...
```{r model-complexity-comparison, fig.cap="模型复杂度与拟合优度平衡：线性、二次、三次和10次多项式模型对植物生物量与土壤养分关系的拟合效果比较"}
# 绘制比较图
...
```{r bird-info-criteria-data, fig.cap="信息准则模型比较：通过ΔAIC和ΔBIC差异比较不同鸟类丰富度模型的相对优劣"}
...
```{r, fig.cap="信息准则可视化：ΔAIC和ΔBIC差异比较"}
...
```{r, fig.cap="林小雨的苗圃实验：温度与光照对植物生长的交互作用。在不同光照强度下温度对植物生长速率的影响，展示了环境因子交互作用在植物生长中的重要性"}
# 可视化交互作用
cat("\n=== 交互作用可视化 ===\n")
...
```{r, fig.cap="林小雨的溪流鱼类模型平均结果：变量重要性和模型权重分布。左图显示水温、溶解氧和pH值是影响鱼类丰度的关键因子，右图展示不同候选模型的相对支持度"}
# 可视化模型平均结果
par(mfrow = c(1, 2))
...
```{r, fig.cap="林小雨的森林鸟类模型10折交叉验证：RMSE在不同数据子集上的变化。图中显示RMSE在不同折之间相对稳定，表明模型具有良好的泛化能力"}
# 可视化交叉验证结果
library(ggplot2)
...
```{r, fig.cap="林小雨的森林生态系统外部验证：训练集和测试集上植物物种丰富度模型的预测性能比较。训练集基于某森林区域数据，测试集代表生态条件不同的另一森林区域"}
# 可视化外部验证结果
library(ggplot2)
...
```{r, fig.cap="林小雨的森林模型残差诊断图：残差vs拟合值、Q-Q图、尺度-位置图和残差vs杠杆图。通过系统诊断，林小雨检查她的树木生长速率模型是否满足统计假设。"}
# 林小雨执行全面的模型诊断
cat("=== 林小雨的森林模型诊断结果 ===\n")
...
```{r, fig.cap="Cook's Distance影响分析：识别对模型参数估计有过度影响的观测点"}
# 可视化影响分析
library(ggplot2)
...
```{r, fig.cap="贝叶斯预测：观测值与预测值的比较，包含95%预测区间", message=FALSE, warning=FALSE}
# 贝叶斯预测评估演示
library(brms)
