...
barplot(forest_bird_model_comparison$delta_AIC,
  ...
  main = expression(paste("常林的鸟类模型", Delta, "AIC差异比较")),
...
barplot(forest_bird_model_comparison$delta_BIC,
  names.arg = forest_bird_model_comparison$Model,
  main = expression(paste("常林的鸟类模型", Delta, "BIC差异比较")),
...
根据信息准则的分析结果，常林得出了重要的模型选择启示。在AIC准则下，最优模型为`r best_aic`（AIC权重 = `r round(forest_bird_model_comparison$AIC_weight[1], 3)`），表明在候选模型中，该模型的相对证据强度最高；在BIC准则下，最优模型为`r best_bic`，因为BIC对参数个数的惩罚更重，倾向于选择更简约的模型。值得注意的是，`r if(best_aic == best_bic) { "AIC和BIC在此例中指向同一个最优模型，这增强了我们对该模型选择的信心。" } else { paste0("AIC最优模型（", best_aic, "）与BIC最优模型（", best_bic, "）在此例中不同，这提示我们信息准则的选择需要结合研究目标：若以预测为导向，倾向于AIC的选择；若以识别核心机制为导向，BIC的选择可能更有参考价值。") }` 过度拟合模型虽然R²更高，但信息准则对其过高的复杂度施加了严厉惩罚——这是信息准则保护我们免于选择过拟合模型的机制所在。
...
```{r nursery-interaction-plot, fig.cap="常林的苗圃实验：温度与光照对植物生长速率的交互作用，展示了环境因子交互作用在植物生长中的重要性", echo=FALSE}
# 可视化交互作用
cat("\n=== 交互作用可视化 ===\n")
...
```{r stream-model-averaging-plot, fig.cap="常林的溪流鱼类模型平均结果：变量重要性排序和模型权重分布。左图显示各环境因子的相对重要性，右图展示不同候选模型的相对支持度", echo=FALSE}
# 可视化模型平均结果
par(mfrow = c(1, 2))
...
从图\@ref(fig:stream-model-averaging-plot)左图中可以观察到各环境因子的变量重要性排序：`r paste(names(sort(stream_var_importance, decreasing=TRUE))[1:3], collapse="、")`的重要性最高，表明它们是影响溪流鱼类丰度的关键驱动因子，这与生态学理论中水温、溶解氧和pH对水生生物分布的决定性作用相符。右图的模型权重分布显示没有单一模型占据绝对优势，多个模型都获得了一定的支持度，这体现了模型平均的必要性——如果仅选"最优"模型，我们将丢失其他合理模型所包含的信息。这种可视化方式使得复杂的模型平均结果变得直观易懂，为生态学决策提供了清晰的依据。
...
```{r, echo=FALSE}
cat("常林的溪流测试条件：水温18°C, 溶解氧6mg/L, pH7.5, 浊度20NTU\n")
...
```{r forest-cv-rmse-plot, fig.cap="常林的森林鸟类模型10折交叉验证：各折RMSE的变化，红色虚线为平均RMSE。通过各折之间的波动程度评估模型的稳定性", echo=FALSE}
# 可视化交叉验证结果
...
```{r external-validation-plot, fig.cap="常林的森林生态系统外部验证：训练集和测试集上植物物种丰富度模型的预测性能比较。训练集基于某森林区域数据，测试集代表生态条件不同的另一森林区域", echo=FALSE}
# 可视化外部验证结果
