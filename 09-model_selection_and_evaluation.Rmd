...
为了更直观地展示模型比较结果，以下展示信息准则可视化图（图\@ref(fig:forest-bird-info-criteria-plot)）。该图采用双面板布局，左侧展示ΔAIC比较，右侧展示ΔBIC比较。图中使用三色编码辅助判断：绿色表示强支持模型（ΔAIC/ΔBIC < 2），黄色表示有实质性差异的模型（2 ≤ ΔAIC/ΔBIC < 7），红色表示支持度较弱的模型（ΔAIC/ΔBIC ≥ 7，其中7-10为过渡地带，>10时基本无支持）。两条虚线分别标示了ΔAIC/ΔBIC为2和7的阈值。注意三色分类是简化的视觉辅助，精确解读时应参考上文给出的完整阈值区间。
...
从图\@ref(fig:forest-bird-info-criteria-plot)中可以清晰地观察到，`r best_aic`模型在AIC和BIC准则下都表现最优（绿色柱状图），而过度拟合模型虽然R²较高，但由于参数过多受到了信息准则的惩罚（红色柱状图）。这种可视化方式使得模型比较结果更加直观易懂，读者可以快速识别出统计上最优且生态学意义明确的模型。

根据信息准则的分析结果，可以得出以下模型选择启示。在AIC准则下，最优模型为`r best_aic`（AIC权重 = `r round(forest_bird_model_comparison$AIC_weight[1], 3)`），表明在候选模型中，该模型的相对证据强度最高；在BIC准则下，最优模型为`r best_bic`，因为BIC对参数个数的惩罚更重，倾向于选择更简约的模型。值得注意的是，`r if(best_aic == best_bic) { "AIC和BIC在此例中指向同一个最优模型，这增强了对该模型选择的信心。" } else { paste0("AIC最优模型（", best_aic, "）与BIC最优模型（", best_bic, "）在此例中不同，这提示信息准则的选择需要结合研究目标：若以预测为导向，倾向于AIC的选择；若以识别核心机制为导向，BIC的选择可能更有参考价值。") }` 过度拟合模型虽然R²更高，但信息准则对其过高的复杂度施加了严厉惩罚——这是信息准则防止选择过拟合模型的机制所在。
...
根据似然比检验的结果（p值 = `r round(nursery_lrt_p_value, 4)`），可得出重要的生态学解释。`r if(nursery_lrt_p_value < 0.05) { "似然比检验显著 (p < 0.05)，表明温度与光照的交互作用对植物生长有显著影响，复杂模型显著改善了模型拟合，应该选择包含交互项的模型。" } else { "似然比检验不显著 (p > 0.05)，表明温度与光照的交互作用对植物生长影响不显著，复杂模型没有带来统计显著的改善，应该选择更简约的简单模型。" }`
