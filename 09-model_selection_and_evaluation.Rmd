...
```{r forest-diagnostic-data-generation, echo=FALSE}
# 林小雨的森林模型诊断研究 - 加载预生成数据（种子: 4040）
load("data/forest_diagnostic_data.rds")
```

```{r forest-diagnostic-data-loading}
# 加载林小雨的森林模型诊断数据
load("data/forest_diagnostic_data.rds")
...
```{r model-diagnostic-residuals, fig.cap="林小雨的森林模型残差诊断图：残差vs拟合值、Q-Q图、尺度-位置图和残差vs杠杆图。通过系统诊断，林小雨检查她的树木生长速率模型是否满足统计假设。"}
...
```{r cooks-distance-plot, fig.cap="Cook's Distance影响分析：识别对模型参数估计有过度影响的观测点。数据点使用蓝色菱形，阈值线使用红色虚线", echo=FALSE}
# 可视化影响分析

# Cook's distance图
cook_data <- data.frame(Observation = 1:n_forest_trees, CooksD = cooks_d)
ggplot(cook_data, aes(x = Observation, y = CooksD)) +
  geom_point(color = "blue", shape = 18, size = 2) +  # 蓝色菱形点
  geom_hline(yintercept = 4 / (n_forest_trees - p),
    linetype = "dashed", color = "red") +
  labs(
...
<!--
`r if(length(high_leverage) > 0) {"- 高杠杆点可能代表极端环境条件（如全光照但贫瘠土壤）\n- 这些观测对模型参数估计有不成比例的影响\n"}`

`r if(length(outlier_residuals) > 0) {"- 异常残差点可能代表特殊的生态情境\n- 例如，异常高生长速率可能出现在人工林或施肥样地\n- 异常低生长速率可能出现在火烧迹地或病虫害样地\n"}`

`r if(length(influential_points) > 0) {"- 强影响点可能显著改变模型的生态学结论\n- 需要仔细检查这些观测的生态学合理性\n"}`
-->
...
### 练习二：贝叶斯模型选择与频率学派方法比较——天童山水生生态对比

**背景**：除了森林样地研究外，林小雨还参与了一个对比研究，天童山溪流生态系统中鱼类丰度与环境因子的关系。数据包括80个溪流样点的鱼类丰度以及水温、溶解氧、pH值、浊度等环境因子。这个水生系统与森林系统形成有趣的对比：溪流中环境因子之间的关系更为线性，而森林系统中非线性关系更加普遍。

**任务**：本练习要求你分别使用频率学派和贝叶斯方法进行分析。在频率学派分析中，使用逐步回归方法选择重要环境因子，计算AIC和BIC值确定最优模型，并进行模型诊断和交叉验证。在贝叶斯分析中，使用贝叶斯线性回归拟合模型，计算贝叶斯因子比较不同模型，执行贝叶斯模型平均计算变量后验包含概率，生成后验预测分布和95%可信区间。然后比较频率学派和贝叶斯方法在变量选择结果上的异同，分析两种方法在不确定性量化方面的差异，讨论两种方法在生态学解释上的优势和局限性。最后基于分析结果，识别影响天童山溪流鱼类丰度的关键环境因子，联系森林-溪流耦合的生态过程提出综合保护建议。

**思考题**：在什么情况下贝叶斯方法比频率学派方法更适合生态学研究？如何合理设置贝叶斯分析中的先验分布，如果天童山有5年前的水质-鱼类关系研究，如何将已有知识编码为先验？

### 练习三：天童山鸟类保护的模型评估与决策支持

**背景**：林小雨的最终任务是为天童山自然保护区制定鸟类保护策略。她拥有保护区过去5年的鸟类调查数据以及详细的生境特征数据（林冠结构、林下植被盖度、枯立木密度、距水源距离等）。她需要构建鸟类丰富度的预测模型，以此指导保护区的功能区划和管理决策。

**任务**：本练习要求你帮助林小雨完成她的研究。构建多个候选模型预测鸟类丰富度，使用信息准则选择最优模型，并解释最优模型的生态学意义，哪些生境特征对天童山鸟类多样性最为关键？进行内部验证，使用k折交叉验证评估模型泛化能力，特别注意天童山样地的空间结构，是否应该使用空间分块CV而非随机CV？分析模型在不同数据子集上的稳定性。进行外部验证，使用天童山邻近区域的独立调查数据验证模型，分析模型在不同生境类型中的预测表现。进行不确定性量化，计算预测置信区间，使用Bootstrap方法量化预测不确定性，分析不确定性来源。最后提供保护决策支持，基于模型预测识别天童山鸟类保护优先区域，评估不同管理措施（如保留枯立木、控制林冠开度）对鸟类丰富度的潜在影响，向保护区管理者呈报一份同时包含"最佳估计"和"最坏情况"的管理建议。

**思考题**：在向保护区管理者汇报模型结果时，林小雨如何平衡模型的预测精度与诚实的不确定性陈述？如果外部验证显示模型在河谷生境中预测良好但在山脊生境中表现差，这是模型的失败还是值得研究的生态学信号？
