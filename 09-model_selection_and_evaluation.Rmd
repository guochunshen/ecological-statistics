...
```{r forest-bird-info-criteria-plot, fig.cap="信息准则可视化：ΔAIC和ΔBIC差异比较", echo=FALSE}
...
```{r nursery-interaction-plot, fig.cap="林小雨的苗圃实验：温度与光照对植物生长的交互作用。在不同光照强度下温度对植物生长速率的影响，展示了环境因子交互作用在植物生长中的重要性", echo=FALSE}
# 可视化交互作用
cat("\n=== 交互作用可视化 ===\n")
...
```{r cooks-distance-plot, fig.cap="Cook's Distance影响分析：识别对模型参数估计有过度影响的观测点", echo=FALSE}
# 可视化影响分析
library(ggplot2)
...
```{r bayesian-prediction-plot, fig.cap="贝叶斯预测：观测值与预测值的比较，包含95%预测区间", message=FALSE, warning=FALSE}
# 贝叶斯预测评估演示
library(brms)
