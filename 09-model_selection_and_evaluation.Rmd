...
forest_biomass_true <- 50 + 0.8 * forest_soil_nutrient - 0.005 * forest_soil_nutrient^2
forest_biomass_observed <- forest_biomass_true + rnorm(n_forest_plots, 0, 10)
forest_soil_data <- data.frame(
  soil_nutrient = forest_soil_nutrient,
  plant_biomass = forest_biomass_observed
)
save(forest_soil_data, file = "data/forest_soil_data.rds")
```
...
```{r model-complexity-comparison, fig.cap="模型复杂度与拟合优度平衡：线性、二次、三次和10次多项式模型对植物生物量与土壤养分关系的拟合效果比较。观测数据点使用蓝色圆形表示，拟合线采用红色实线", echo=FALSE}
# 绘制比较图
...
```{r bird-info-criteria-data, fig.cap="信息准则模型比较：通过ΔAIC和ΔBIC差异比较不同鸟类丰富度模型的相对优劣", echo=FALSE}
# 信息准则在生态学中的应用演示：林小雨的森林鸟类丰富度研究

# 检查数据文件是否存在，如果不存在则生成数据
if (!file.exists("data/forest_bird_data.rds")) {
  set.seed(456)
  ...
  # 保存数据到文件，供后续使用
  saveRDS(forest_bird_data, "data/forest_bird_data.rds")
} 
```
...
```{r forest-bird-info-criteria-plot, fig.cap="信息准则可视化：ΔAIC和ΔBIC差异比较。模型优劣使用颜色和填充模式组合表示：优秀模型（绿色/斜线）、可接受模型（黄色/交叉线）、较差模型（红色/网格线）", echo=FALSE}
...
# 检查数据文件是否存在，如果不存在则生成数据
if (!file.exists("data/nursery_plant_data.rds")) {
  set.seed(789)
  ...
  # 保存数据到文件，供后续使用
  saveRDS(nursery_plant_data, "data/nursery_plant_data.rds")
} 
```
...
```{r nursery-interaction-plot, fig.cap="林小雨的苗圃实验：温度与光照对植物生长的交互作用。在不同光照强度下温度对植物生长速率的影响，展示了环境因子交互作用在植物生长中的重要性。不同光照强度使用颜色和线型组合表示", echo=FALSE}
# 可视化交互作用
cat("\n=== 交互作用可视化 ===\n")
...
# 检查数据文件是否存在，如果不存在则生成数据
if (!file.exists("data/stream_fish_data.rds")) {
  set.seed(101)
  ...
  # 保存数据到文件，供后续使用
  saveRDS(stream_fish_data, "data/stream_fish_data.rds")
...
```{r stream-model-averaging-plot, fig.cap="林小雨的溪流鱼类模型平均结果：变量重要性和模型权重分布。左图显示水温、溶解氧和pH值是影响鱼类丰度的关键因子，右图展示不同候选模型的相对支持度。变量重要性使用蓝色斜线填充，模型权重使用绿色网格填充", echo=FALSE}
# 可视化模型平均结果
par(mfrow = c(1, 2))
...
# 检查数据文件是否存在，如果不存在则生成数据
if (!file.exists("data/forest_plot_data.rds")) {
  set.seed(2024)
  ...
  # 保存数据到文件，供后续使用
  saveRDS(forest_plot_data, "data/forest_plot_data.rds")
...
```{r forest-cv-rmse-plot, fig.cap="林小雨的森林鸟类模型10折交叉验证：RMSE在不同数据子集上的变化。图中显示RMSE在不同折之间相对稳定，表明模型具有良好的泛化能力。数据点使用蓝色方形，连接线使用蓝色实线，平均线使用红色虚线", echo=FALSE}
# 可视化交叉验证结果
library(ggplot2)
...
# 检查数据文件是否存在，如果不存在则生成数据
if (!file.exists("data/external_validation_data.rds")) {
  set.seed(3030)
  ...
  # 将训练集和测试集打包成一个列表并保存到文件
  external_validation_data <- list(
    train_data = train_data,
    test_data = test_data
  )

  # 保存数据到文件，供后续使用
  saveRDS(external_validation_data, "data/external_validation_data.rds")
} 
```
...
```{r external-validation-plot, fig.cap="林小雨的森林生态系统外部验证：训练集和测试集上植物物种丰富度模型的预测性能比较。训练集基于某森林区域数据，测试集代表生态条件不同的另一森林区域。训练集使用蓝色圆形点，测试集使用红色三角形点", echo=FALSE}
# 可视化外部验证结果
library(ggplot2)
...
# 林小雨的森林模型诊断研究 - 数据生成
# 检查数据文件是否存在，如果不存在则生成数据
if (!file.exists("data/forest_diagnostic_data.rds")) {
  set.seed(4040)
  ...
  # 保存数据到文件
  save(forest_diagnostic_data, n_forest_trees, file="data/forest_diagnostic_data.rds")
}
```
...
```{r bayesian-model-data-generation, echo=FALSE}
# 贝叶斯模型选择演示 - 数据生成
# 检查数据文件是否存在，如果不存在则生成数据
if (!file.exists("data/forest_bird_data_bayes.rds")) {
  set.seed(5050)
  ...
  # 保存数据到文件
  save(forest_bird_data_bayes, n_forest_birds, file="data/forest_bird_data_bayes.rds")
}
```
...
贝叶斯因子比较结果显示，模型2相对于模型1的贝叶斯因子为 `r round(exp(bf_12@bayesFactor$bf), 2)`，模型3相对于模型2的贝叶斯因子为 `r round(exp(bf_23@bayesFactor$bf), 2)`。
...
```{r bayesian-prediction-plot, fig.cap="贝叶斯预测：观测值与预测值的比较，包含95%预测区间。数据点使用蓝色三角形，预测区间使用浅蓝色误差线，参考线使用红色虚线", message=FALSE, warning=FALSE}
# 贝叶斯预测评估演示
library(brms)
