...
```{r, fig.cap="模型复杂度与拟合优度的平衡：比较线性、二次、三次和10次多项式模型对植物生物量与土壤养分关系的拟合效果"}
# 演示模型复杂度与拟合优度的平衡
# 设置随机种子确保结果可重现
set.seed(123)
...
```{r, fig.cap="模型复杂度与拟合优度平衡：线性、二次、三次和10次多项式模型对植物生物量与土壤养分关系的拟合效果比较"}
# 绘制比较图
...
```{r, fig.cap="信息准则模型比较：通过ΔAIC和ΔBIC差异比较不同鸟类丰富度模型的相对优劣"}
# 信息准则在生态学中的应用演示
set.seed(456)
...
根据信息准则的分析结果，我们可以得出重要的模型选择启示。根据$\text{AIC}$准则，最优模型是`r best_aic`，其$\text{AIC}$权重为`r round(model_comparison$AIC_weight[1], 3)`，表明这个模型在候选模型中最有可能。根据$\text{BIC}$准则，最优模型是`r best_bic`，$\text{BIC}$倾向于选择更简约的模型。模型选择启示表明，`r best_aic`模型在$\text{AIC}$和$\text{BIC}$下都表现良好，这个模型包含了真实关系中的关键变量，而过度拟合模型虽然R²更高，但信息准则惩罚了其复杂度。
...
```{r, fig.cap="温度与光照对植物生长的交互作用：在不同光照强度下温度对植物生长速率的影响"}
# 可视化交互作用
cat("\n=== 交互作用可视化 ===\n")
...
根据似然比检验的结果（p值 = `r round(lrt_p_value, 4)`），我们可以得出重要的生态学解释。`r if(lrt_p_value < 0.05) { "似然比检验显著 (p < 0.05)，表明温度与光照的交互作用对植物生长有显著影响，复杂模型显著改善了模型拟合，应该选择包含交互项的模型。" } else { "似然比检验不显著 (p > 0.05)，表明温度与光照的交互作用对植物生长影响不显著，复杂模型没有带来统计显著的改善，应该选择更简约的简单模型。" }`
...
```{r, fig.cap="10折交叉验证：RMSE在不同数据子集上的变化"}
# 可视化交叉验证结果
library(ggplot2)
...
```{r, fig.cap="外部验证：训练集和测试集上植物物种丰富度模型的预测性能比较"}
# 外部验证在生态学中的应用演示
set.seed(3030)
...
```{r, fig.cap="模型残差诊断图：残差vs拟合值、Q-Q图、尺度-位置图和残差vs杠杆图"}
# 执行全面的模型诊断
cat("=== 模型诊断结果 ===\n")
...
`r if(length(high_leverage) > 0) {"- 高杠杆点可能代表极端环境条件（如极高水温、极低溶解氧）\n- 这些观测对模型参数估计有不成比例的影响\n"}`
