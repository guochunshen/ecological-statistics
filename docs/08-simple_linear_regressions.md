...
\caption{不同R²值的拟合效果对比：左(R²=0.1)弱、中(R²=0.5)中等、右(R²=0.9)强。}(\#fig:unnamed-chunk-4)
\end{figure}
...
\caption{R²与调整R²随自变量数量变化的对比：调整R²对无关变量施加惩罚，防止过度拟合。}(\#fig:r2-adj-r2-comparison)
\end{figure}
...
\caption{整体模型显著性比较：显著模型（左）与不显著模型（右）的对比。}(\#fig:model-significance-comparison)
\end{figure}
...
**解析解与梯度下降：殊途同归**。线性回归之所以在统计学中享有特殊地位，很大程度上是因为它拥有解析解。通过正态方程（Normal Equation）：
...
重要的是认识到：解析解和梯度下降求解的是**同一个优化问题**，只是解法不同。这就像我们既可以用求根公式解一元二次方程，也可以用牛顿迭代法逐步逼近，两种方法给出相同的答案（在数值精度范围内），但适用的场景截然不同。

**核心启示：一个连续谱系而非二元选择**。线性回归是深度学习的特例，具体来说，是单层、无激活函数、使用均方误差损失的特例。深度学习则是线性回归的非线性推广，通过堆叠多层、引入非线性激活函数（如ReLU、sigmoid、tanh）、使用任意损失函数，将线性模型扩展为能够逼近任意复杂函数的强大工具。
...
> **尝试与AI协作**
>
> 1. 请AI用Python（PyTorch或TensorFlow）实现一个单层线性神经网络（无激活函数，使用MSELoss），并在你熟悉的生态数据集上训练。比较其参数估计结果与R语言`lm()`函数的输出是否一致（在数值精度范围内）。
> 2. 向AI提问：当特征数$p$很大时（例如$p=10000$），正态方程和梯度下降的计算代价分别是多少？在什么条件下，梯度下降会收敛到与解析解完全相同的结果？
>
> **批判性评估**：AI可能声称"线性回归和神经网络完全等价"，这是对数学结构的描述，但两者的推断框架不同：线性回归附带完整的假设检验体系（$t$检验、$F$检验、置信区间、预测区间），而直接使用神经网络框架时通常只关注预测精度。在生态学研究中，如果我们关心的不仅是预测还有统计推断（例如：某个环境因子是否显著影响物种分布？该效应的95%置信区间是多少？），线性回归的经典推断框架可能比纯预测导向的神经网络更直接适用。两种视角各有其价值，不互相替代。
...
\caption{变量相对重要性柱状图：使用LMG方法计算的各预测变量对模型解释的相对贡献度}(\#fig:variable-importance-plot)
\end{figure}
...
\caption{物种丰富度与海拔关系的多项式回归：线性与二次多项式的拟合对比。}(\#fig:unnamed-chunk-36)
\end{figure}
...
\caption{多项式回归的过度拟合演示：低次与高次多项式的泛化能力对比。}(\#fig:overfitting-demo)
\end{figure}
...
**林小雨学到了什么**：回到天童山的样地，林小雨用多元回归模型来预测树木的年生长量。她将光照强度、土壤含水量、氮含量以及邻近树木的竞争指数作为自变量输入模型。通过AIC比较，她发现光照和竞争指数是驱动生长的最关键因子，而土壤含水量在多个变量共存时其独立贡献并不明显，这让她理解了"单变量显著不等同于多变量场景中仍然重要"的关键洞察。回归诊断中，她发现低光照样方的残差异常偏大，追溯原因后意识到这些样方曾遭受虫害，这不是模型的问题，而是数据中隐藏了未被记录的干扰事件。多项式回归让她捕捉到了光照对生长速率的非线性饱和效应，当光照超过某个阈限后，生长的边际增益开始递减，这比直线假设更能反映植物光合作用的生理规律。

**统计-AI桥梁**：林小雨写下$y = X\beta + \varepsilon$后意识到，这正是只有一个输出层且没有激活函数的神经网络！回归系数$\beta$就是网络的"权重"，最小二乘法等价于以均方误差为损失函数的训练过程。她由此理解了深度学习的双重基因：正向继承了回归将多元输入映射为标量输出的基本框架，反向则是通过堆叠隐藏层和非线性激活将这种线性映射的容量拓展到极致。双重下降现象让她对"过拟合"有了更细腻的认识，在参数量超过样本量的某个深处，测试误差可能不升反降，这意味着"越多参数越危险"不是绝对的铁律。

**生态学意义**：回归模型不仅帮助林小雨预测了树木的生长速率，更重要的是让她知道了哪些生态因子在驱动这片森林的动态变化。知道自己不知道什么，残差中的未解释变异、在多元分析中变得不再显著的变量，这本身就是对生态系统运行机制最深刻的洞察。

## 综合练习

### 练习1：光照对天童山树木年生长量的回归分析

**背景**：林小雨在天童山20公顷样地中测量了50棵树木的年生长量（cm/年），同时记录了每棵树冠层上方的光照强度（μmol/m²/s）。她希望通过回归分析来量化光照对树木生长的驱动作用，并与单纯依赖经验判断的传统方法进行对比。
...
### 练习2：天童山树木生长量的多因子驱动分析

**背景**：林小雨意识到光照只是影响树木生长的众多因子之一。她收集了天童山样地中100棵树木的数据，包括年生长量（cm/年）、光照强度、土壤氮含量（g/kg）、土壤含水量（%）以及邻体竞争指数（基于邻近树木的胸径和距离计算）。她希望识别哪些因子是生长的关键驱动力。

**数据文件**：`data/bird_data_p1.Rdata`



**任务**：使用多元线性回归分析所有环境因子对树木生长量的综合影响。使用逐步回归进行变量选择，识别最重要的驱动因子。比较完整模型与简化模型的AIC、R²和调整R²。进行多重共线性诊断，光照和土壤含水量可能高度相关（林冠开阔处光照强但土壤易干燥），计算VIF。对最终模型进行回归诊断，特别注意生长量最大的几棵"超级树"是否是高杠杆或异常值。

**思考问题**：当光照和土壤含水量高度相关时，回归系数的解释会受什么影响？为什么单因子分析中显著的变量在多因子分析中可能变得不显著？如何根据回归结果向保护区管理者解释哪些环境因素真正在驱动森林生产力？

### 练习3：树木胸径与树高的非线性关系建模

**背景**：森林碳储量的估算通常依赖树木的胸径-树高关系。林小雨在天童山样地中测量了80棵不同树种树木的胸径（cm）和树高（m）。林业经验告诉她，树高随胸径增长的速度会逐渐减缓，大树的高度增长比幼树慢得多。

**数据文件**：`data/lake_data_p1.Rdata`



**任务**：使用简单线性回归分析胸径与树高的关系，绘制散点图及回归线。尝试对数变换（log-log模型）和二次多项式回归来描述胸径-树高的非线性关系。比较三种模型（线性、log-log、二次多项式）的拟合效果和残差分布。基于最佳模型，预测胸径为50 cm的树木的树高，并与胸径10 cm的树木比较生长速率差异。

**思考问题**：为什么胸径-树高关系在生态学中几乎总是非线性的？log变换的生态学解释是什么（异速生长方程）？当使用log变换后的模型做预测时，如何正确地将预测值转回原始尺度（注意反变换的偏差校正）？
