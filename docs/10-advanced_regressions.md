# 复杂生态关系：高级回归模型与AI融合 {#ch10}



## 本章学习目标

**知识目标**：学完本章后，你能回答——GAM的平滑项和GLM的link函数各解决什么问题？什么时候用混合效应模型而不是固定效应模型？空间自相关为什么不能忽略？

**技能目标**：你能独立用R的`mgcv`包拟合GAM模型并诊断平滑项，用`lme4`包构建混合效应模型，用`spdep`包检验和建模空间自相关，以及用`lavaan`包构建简单的结构方程模型。

**AI素养目标**：你能理解GAM的样条平滑与神经网络隐式平滑之间的概念联系，GLM的link函数与神经网络输出激活函数的数学同构关系，以及混合效应模型的随机效应与层次贝叶斯神经网络的分层结构之间的对应。你将能够判断何时传统高级回归足够，何时需要神经网络。
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/visualize-forest-am-1} 

}

\caption{林小雨的森林调查：树木生长速率与环境因子关系的可加性模型平滑效应图}(\#fig:visualize-forest-am)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/diagnose-forest-am-1} 

}

\caption{林小雨的森林调查：树木生长速率可加性模型诊断图}(\#fig:diagnose-forest-am)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-2-1} 

}

\caption{森林树木生长可加性模型平滑复杂度诊断图}(\#fig:unnamed-chunk-2-1)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-2-2} 

}

\caption{森林树木生长可加性模型平滑复杂度诊断图}(\#fig:unnamed-chunk-2-2)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/residual-vs-fitted-1} 
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/qq-plot-residuals-1} 
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/residual-vs-temperature-1} 
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/acf-residuals-1} 
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/visualize-prediction-intervals-1} 

}

\caption{温度对树木生长速率影响的预测曲线（含95\%置信区间）}(\#fig:visualize-prediction-intervals)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/visualize-plant-survival-1} 

}

\caption{土壤湿度对森林植物存活率影响的预测曲线（含95\%置信区间）}(\#fig:visualize-plant-survival)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-6-1} 

}

\caption{森林物种分布逻辑回归模型残差诊断图}(\#fig:unnamed-chunk-6)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-8-1} 

}

\caption{森林物种分布逻辑回归模型ROC曲线}(\#fig:unnamed-chunk-8)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-11-1} 

}

\caption{森林物种分布逻辑回归模型影响点诊断图}(\#fig:unnamed-chunk-11-1)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-11-2} 

}

\caption{森林物种分布逻辑回归模型影响点诊断图}(\#fig:unnamed-chunk-11-2)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-11-3} 

}

\caption{森林物种分布逻辑回归模型影响点诊断图}(\#fig:unnamed-chunk-11-3)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-12-1} 

}

\caption{温度对森林物种出现概率影响的预测曲线（含95\%置信区间）}(\#fig:unnamed-chunk-12)
\end{figure}
...
### 与AI/ML的数学连接：连接函数与输出激活函数

广义线性模型（GLM）中连接函数的概念，在现代神经网络中有一个精确的对应物——输出层的激活函数。理解这一对应关系，不仅能帮助我们建立统计学与机器学习的统一视角，还能为模型选择提供更深层的理论指导。

**统计概念回顾**。在GLM框架中，我们通过三个组件描述数据生成过程：随机部分（响应变量的分布族）、系统部分（线性预测器 $\eta = X\beta$）和连接函数（将线性预测器与分布参数联系起来的函数 $g(\mu) = \eta$）。连接函数的选择取决于响应变量的性质：连续正数通常使用对数连接，比例数据使用logit连接，计数数据使用对数连接。连接函数的作用是将线性预测器的无界输出映射到响应变量的合理取值空间。

**AI/ML数学连接**。在神经网络中，输出层激活函数完成的任务与GLM的连接函数完全等价。sigmoid函数 $\sigma(x) = 1/(1+e^{-x})$ 将任意实数值映射到(0,1)区间，这正是logit连接的逆函数——换言之，使用sigmoid输出的神经网络等价于一个使用伯努利分布和logit连接的GLM，但线性预测器不再是原始特征的线性组合，而是通过多层非线性变换得到的隐式特征表示。softmax函数在多分类问题中将任意实向量映射为概率单纯形，这与多元logit连接函数在数学上等价。线性激活函数（恒等函数）对应正态分布的身份连接，指数激活函数对应泊松分布的对数连接，ReLU虽然不直接对应任何标准GLM连接函数，但其形式 $f(x) = \max(0, x)$ 可以被理解为一种数据驱动学习出来的、适合处理非负响应的分段线性连接函数。

这一对应关系的深层含义在于：选择神经网络的输出激活函数和损失函数，在本质上等价于为数据选定一个概率模型。使用sigmoid输出加交叉熵损失进行二分类，等价于使用伯努利分布假设下的最大似然估计；使用线性输出加均方误差损失进行回归，等价于正态分布假设下的最大似然估计。这种等价性揭示了神经网络并非完全"无模型"的黑箱——它的输出层隐含了一个明确的概率分布假设，和GLM的框架一脉相承。

**生态学场景**。在森林碳储量预测中，生物量始终为正值且通常右偏，这意味着无论使用GLM还是神经网络，输出层都应当选择能确保正值输出的连接函数/激活函数（如指数函数或softplus）。在物种出现概率建模中，二项分布和logit连接（或sigmoid输出）是自然的选择。理解这一对应关系后，生态学家可以在GLM中利用已有生态学知识选择恰当的分布族，然后将相同的分布假设延续到神经网络的输出层设计中，实现统计知识与AI工具的有效衔接。

**批判性反思**。尽管GLM的连接函数与神经网络的输出激活函数在数学上具有等价性，但在方法论哲学上二者存在本质区别。GLM要求研究者在分析开始前就明确指定分布族和连接函数，这种"先验假设"的框架鼓励对数据生成过程的深入思考，但可能在假设不成立时导致系统性偏差。神经网络通过端到端学习和隐藏层的非线性变换，能够在输出层之外"补偿"部分连接函数选择的偏差，但其代价是模型可解释性的丧失和过拟合风险的增加。因此，一个理想的方案是：用生态学知识指导网络输出层的设计（就像选择GLM的分布族），同时利用隐藏层的灵活性捕捉数据中未被先验知识涵盖的复杂模式。
...
\begin{center}\includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/visualize-forest-bird-gam-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/visualize-forest-bird-gam-2} \end{center}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/visualize-forest-bird-prediction-1} 

}

\caption{林小雨的森林调查：鸟类物种丰富度与环境因子关系的广义可加模型分析}(\#fig:visualize-forest-bird-prediction)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-15-1} 

}

\caption{森林鸟类丰富度广义可加模型平滑复杂度诊断图}(\#fig:unnamed-chunk-15-1)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-15-2} 

}

\caption{森林鸟类丰富度广义可加模型平滑复杂度诊断图}(\#fig:unnamed-chunk-15-2)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-16-1} 

}

\caption{森林鸟类丰富度广义可加模型残差诊断图}(\#fig:unnamed-chunk-16)
\end{figure}
...
\begin{center}\includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-18-1} \end{center}
...
### 与AI/ML的数学连接：平滑样条与神经网络的隐式平滑

广义可加模型通过显式的平滑函数（样条）来捕捉非线性关系，而神经网络通过隐藏层的组合实现隐式的非线性拟合。这两种方法虽然来自不同的数学传统，但在函数逼近的核心目标上有着深刻的共通性。

**统计概念回顾**。GAM的核心思想是将响应变量表示为预测变量平滑函数的和：$y = \beta_0 + \sum_{j=1}^p f_j(x_j) + \varepsilon$。每个 $f_j$ 通常用样条基函数的线性组合表示：$f_j(x) = \sum_{k=1}^K \beta_{jk} B_{jk}(x)$，其中 $B_{jk}$ 是基函数（如薄板样条、B样条），并通过惩罚项控制平滑度，在拟合优度和平滑性之间寻求最优平衡。GCV和REML提供了自动选择平滑参数的方法，避免了主观调整的随意性。

**AI/ML数学连接**。神经网络可以视为一种广义的非线性函数逼近器。每个隐藏层执行的是 $a^{(l)} = f(W^{(l)}a^{(l-1)} + b^{(l)})$，其中 $f$ 是激活函数（如ReLU）。当我们将多个这样的层叠加时，整个网络定义了一个极其灵活的非线性函数空间。与GAM的关键区别在于：GAM将非线性关系分解为各个预测变量的独立平滑函数之和（可加性假设），而神经网络直接学习所有输入变量的联合非线性表示，自动建模变量间的交互效应。从基函数展开的角度看，GAM使用预定义的样条基函数并通过惩罚控制复杂度，而神经网络使用可学习的基函数（由隐藏层权重参数化），并通过随机梯度下降和正则化隐式地控制复杂度。

值得注意的是，某些神经网络架构可以被理解为GAM的推广。例如，神经加性模型（Neural Additive Models, NAM）正是将GAM的可加结构嵌入神经网络框架中：每个预测变量对应一个独立的小型神经网络，各网络的输出相加得到最终预测。这种设计保留了GAM的可解释性（可以单独绘制每个变量的效应曲线），同时获得了神经网络强大的拟合能力。

**生态学场景**。在分析温度对植物物候的非线性影响时，GAM可以生成一张清晰的平滑效应图，直观展示温度在不同区间内如何影响开花时间。生态学家可以据此识别阈值（如低温抑制、适温促进、高温胁迫）并讨论其生物学机制。神经网络可以拟合更复杂的非线性关系（如温度与降水的交互效应），但它产生的隐式函数难以直接可视化。因此，当研究目标是在已知生态学知识框架下检验特定变量的非线性效应时，GAM更具优势；当目标是探索大量变量之间的复杂交互模式时，神经网络更为合适。

**批判性反思**。GAM的优势在于可解释性和统计推断的严谨性：平滑函数可以通过置信带进行不确定性量化，平滑参数的选择有GCV和REML等理论上坚实的基础。然而，GAM的可加性假设和预定义基函数限制了它捕捉变量间复杂交互效应的能力。神经网络通过端到端学习可以自动发现任意形式的非线性关系和交互效应，但其"隐式平滑"的机制难以被人类理解，且容易在数据有限时产生不稳定的外推。一个值得关注的发展方向是将GAM的显式平滑思想与神经网络的表达灵活性相结合——例如，在神经网络中引入样条激活层或可加结构，同时保留部分可解释性和统计推断能力。
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-19-1} 

}

\caption{林小雨的森林调查：混合效应模型诊断图}(\#fig:unnamed-chunk-19-1)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-19-2} 

}

\caption{林小雨的森林调查：混合效应模型诊断图}(\#fig:unnamed-chunk-19-2)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-27-1} 

}

\caption{混合效应模型诊断验证}(\#fig:unnamed-chunk-27-1)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-27-2} 

}

\caption{混合效应模型诊断验证}(\#fig:unnamed-chunk-27-2)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-28-1} 

}

\caption{空间混合模型随机效应分布图}(\#fig:unnamed-chunk-28)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-29-1} 

}

\caption{混合效应模型诊断示例图}(\#fig:unnamed-chunk-29-1)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-29-2} 

}

\caption{混合效应模型诊断示例图}(\#fig:unnamed-chunk-29-2)
\end{figure}
...
### 与AI/ML的数学连接：随机效应与层次贝叶斯神经网络

混合效应模型通过固定效应与随机效应的区分来处理数据的层次结构，这一思想在现代深度学习中有着精确的对应——层次贝叶斯神经网络和元学习。两种方法共享一个核心洞察：在存在组结构的数据中，参数可以被分为共享参数（全局模式）和组特定参数（局部变异）。

**统计概念回顾**。混合效应模型的核心是将总变异分解为固定效应和随机效应：$y = X\beta + Zu + \varepsilon$，其中 $\beta$ 是所有组共享的固定效应参数，$u \sim N(0, \Sigma)$ 是组特定的随机效应，捕捉各组偏离总体均值的程度。这种分解自然地处理了嵌套数据中的组内相关性：同一组的观测共享相同的随机效应 $u$，因此彼此之间具有相关性。随机效应的方差 $\sigma_u^2$ 量化了组间变异的程度，组内相关系数（ICC）则衡量了组内相关占总方差的比例。

**AI/ML数学连接**。在层次贝叶斯神经网络中，核心思想是将某些网络参数（通常是最后几层的权重）设为组特定的随机变量，而其他参数在组间共享。形式上，假设我们有 $K$ 个组（如 $K$ 个森林样地），每个组都有自己的一组参数 $\theta_k$，这些参数不是被独立估计，而是从一个共享的先验分布中抽取：$\theta_k \sim N(\mu, \Sigma)$，其中 $\mu$ 和 $\Sigma$ 在所有组间共享。这一结构与混合效应模型完全对应：$\mu$ 对应固定效应，$\theta_k - \mu$ 对应随机效应。通过反向传播和重参数化技巧（reparameterization trick），神经网络可以同时学习共享参数 $\mu$ 和各组特定的偏移量。

元学习（Meta-Learning）中的MAML（Model-Agnostic Meta-Learning）算法也体现了类似的层次化思想：目标是为每个任务（组）学习一个好的初始化参数，使得从该初始化出发，仅需少量梯度步就能适应新的任务。这种方式等价于学习一个"典型"模型作为固定效应，同时保留各任务快速调整的能力作为随机效应的变体。

**生态学场景**。在分析多个森林样地的碳储量与环境因子的关系时，混合效应模型可以将环境因子的总体效应设为固定效应（如温度每升高1°C，碳储量平均变化量），而将每个样地的截距偏移设为随机效应（如某些样地因未观测的土壤特性而始终高于或低于总体均值）。层次贝叶斯神经网络可以在保持这一解释框架的同时，利用神经网络的灵活性来捕捉更复杂的非线性关系：例如，温度对碳储量的影响可能在每个样地中具有不同的形态（而非简单的截距偏移），神经网络可以通过让高层的隐藏层权重在样地间变化来实现这一效果，而底层特征提取层在样地间保持不变。

**批判性反思**。混合效应模型的最大优势在于其明确的统计推断框架：随机效应的方差可以通过REML准确估计，固定效应的标准误自动考虑了组内相关性，模型的拟合优度可以通过AIC、BIC等准则进行模型选择。层次贝叶斯神经网络虽然具有更大的灵活性，但其参数估计的不确定性量化更为复杂，通常需要MCMC或变分推断，计算成本更高。当前的研究趋势正在将两种方法的优点结合起来：例如，在生态统计中，广义加性混合模型（GAMM）已经结合了平滑项和随机效应；在机器学习中，深度高斯过程和神经过程（Neural Processes）正在探索将神经网络与层次概率建模融合的路径。
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-34-1} 

}

\caption{森林生态系统空间格局可视化}(\#fig:unnamed-chunk-34)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-35-1} 

}

\caption{鸟类物种丰富度空间预测格局图}(\#fig:unnamed-chunk-35)
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-36-1} 

}

\caption{空间模型残差分布图}(\#fig:unnamed-chunk-36)
\end{figure}
...
### 与AI/ML的数学连接：空间权重矩阵与图神经网络

空间自相关模型通过空间权重矩阵来描述地理单元之间的关系，而图神经网络（GNN）通过邻接矩阵和消息传递机制来处理图结构数据。这两种方法在数学结构上高度同源，共享着同一个核心思想：空间上邻近（或图上相连）的节点在特征上趋于相似。

**统计概念回顾**。空间自相关模型的核心是空间权重矩阵 $W$，其中 $W_{ij}$ 表示空间单元 $i$ 和 $j$ 之间的空间关联强度。$W$ 通常基于距离阈值、k近邻或共享边界来定义。空间滞后模型（SAR）的形式为 $y = \rho W y + X\beta + \varepsilon$，其中 $\rho$ 是空间自相关系数，$Wy$ 是空间滞后项，表示邻近单元响应变量值的加权平均。空间误差模型（SEM）将空间自相关置于残差中：$y = X\beta + u$，$u = \lambda W u + \varepsilon$。这两种模型分别对应了空间依赖的两种可能机制：实质性的空间扩散（SAR）和未观测的空间异质性（SEM）。

**AI/ML数学连接**。在图神经网络中，每一层的核心操作是图卷积（Graph Convolution），其最简单的形式为：$H^{(l+1)} = \sigma(\tilde{D}^{-\frac{1}{2}} \tilde{A} \tilde{D}^{-\frac{1}{2}} H^{(l)} W^{(l)})$，其中 $\tilde{A} = A + I$ 是添加自环的邻接矩阵，$\tilde{D}$ 是归一化用的度矩阵，$H^{(l)}$ 是第 $l$ 层的节点特征矩阵，$W^{(l)}$ 是可学习的权重矩阵。这个公式的本质是：每个节点的新表示是其邻居节点表示的加权聚合——注意这个结构与空间滞后模型 $Wy$ 在数学上是同构的！邻接矩阵 $A$ 起到了空间权重矩阵 $W$ 的作用，图卷积的加权聚合步骤等价于空间滞后项的计算。

两者的区别在于深度和灵活性：GNN通过堆叠多层图卷积来聚合多阶邻居的信息（一阶邻居、二阶邻居等），这与空间统计中的高阶空间自回归结构（SARAR模型）在数学上对应。此外，GNN中的权重矩阵 $W^{(l)}$ 是可学习的，可以自动学习每个邻居特征维度的重要性，而传统空间权重矩阵 $W$ 通常需要预先设定。新兴的图注意力网络（GAT）更进一步，通过自注意力机制动态计算节点间的空间权重：$W_{ij} = \text{softmax}_j(\text{LeakyReLU}(a^T[W h_i || W h_j]))$，使得空间权重变为数据驱动的而非预先固定的。

**生态学场景**。在分析森林斑块之间的物种扩散时，空间权重矩阵可以基于斑块之间的地理距离或栖息地连通性来定义。SAR模型可以检验斑块的物种丰富度是否受到邻近斑块丰富度的影响（空间溢出效应）。图神经网络可以更进一步：将每个森林斑块视为图中的一个节点，节点特征包括面积、海拔、植被类型等，边的权重基于斑块间的距离或景观连通性。通过多层图卷积，模型可以学习到不同空间尺度上的生态过程——第一层卷积聚合直接相邻斑块的信息（对应短距离扩散），第二层卷积聚合间接相邻斑块的信息（对应长距离扩散或阶梯式迁移）。

**批判性反思**。传统空间统计模型的优势在于其完善的统计推断框架：Moran's I检验、空间自相关系数的显著性检验、以及空间权重矩阵选择的模型诊断方法都经过了几十年的检验和优化。这些方法在样本量有限时稳健，且结果具有明确的生态学解释。GNN虽然灵活性更高，但其"黑箱"特性使得空间依赖的结构难以解释，且在小样本条件下容易过拟合。一个值得探索的方向是将空间统计的领域知识（如Tobler第一定律——"万物皆相关，但近处的事物比远处的事物更相关"）作为归纳偏置融入GNN的架构设计中，同时利用GNN的表征学习能力捕捉数据中的非线性和高阶空间交互效应。
...
{\centering \includegraphics[width=0.8\linewidth]{10-advanced_regressions_files/figure-latex/unnamed-chunk-40-1} 

}

\caption{森林生态系统结构方程模型路径图}(\#fig:unnamed-chunk-40)
\end{figure}
...
### 与AI/ML的数学连接：结构方程模型与结构因果模型

结构方程模型（SEM）为检验因果假设提供了统计框架，而Judea Pearl提出的结构因果模型（SCM）为因果推断提供了更严格的数学基础。机器学习领域近年来兴起的因果机器学习，正在将两者融合，创造出既具有统计严谨性又具有识别复杂因果效应能力的新方法。

**统计概念回顾**。SEM通过测量模型和结构模型来检验因果关系。测量模型将潜变量与观测指标联系起来：$x = \Lambda_x \xi + \delta$，其中 $\xi$ 是潜变量，$x$ 是观测指标。结构模型描述潜变量之间的因果关系：$\eta = B\eta + \Gamma \xi + \zeta$。通过路径系数可以分解总效应为直接效应和间接效应，从而量化因果路径的重要性。SEM要求研究者基于理论先验地指定因果结构（路径图），然后通过数据检验该结构是否与观测协方差矩阵一致。

**AI/ML数学连接**。Pearl的结构因果模型使用有向无环图（DAG）和结构方程来形式化因果关系：$X_i = f_i(PA_i, U_i)$，其中 $PA_i$ 是变量 $X_i$ 的父节点集合，$U_i$ 是外生噪声。SCM不仅能够表示观测变量之间的关系，还能通过do-演算（do-calculus）进行干预推理和反事实推理——这两个关键能力是传统SEM所不具备的。在机器学习中，因果发现算法（如PC算法、FCI算法）可以从观测数据中自动学习因果结构，而无需完全依赖先验理论。贝叶斯网络在形式上等价于SCM的概率部分，但通常不区分因果关系和统计关联。

深度学习中与SEM最相关的发展是深度结构因果模型（Deep SCM）和因果生成模型。这些模型将神经网络作为因果机制的函数近似器来替代参数化的线性SEM：因果机制 $f_i$ 不再被假定为线性函数，而是由一个神经网络来学习。这使得模型可以捕捉变量之间复杂的非线性因果效应。此外，因果表征学习试图从高维观测数据（如图像、文本）中自动提取因果变量及其因果结构，这可以被视为一个自动化的、非线性的SEM构建过程。

**生态学场景**。在分析气候变化对森林生态系统的因果影响时，传统SEM需要生态学家先验地画出路径图：温度增加→直接影响物种分布，同时→影响土壤水分→间接影响物种分布。如果生态学家对因果结构有充分的理论把握，SEM可以提供清晰的效应分解和模型拟合检验。然而，当涉及的变量很多而理论不够完善时，因果发现算法可以辅助探索可能的因果结构，为SEM的路径图构建提供数据驱动的假设。深度SCM可以进一步处理非线性因果效应，例如温度与物种分布的因果效应可能呈现阈值型而非线性的模式。

**批判性反思**。SEM的核心优势在于其对理论的依赖——这既是优势也是局限。优势在于框架迫使研究者明确表达因果假设，促进理论思考；局限在于当理论不完善时，错误的路径设定会导致有偏的效应估计。因果机器学习试图通过数据驱动的方式发现因果结构，但其方法在当前阶段的可靠性——特别是在小样本和高噪声的生态数据中——还需要更多的验证。do-演算提供了理论上完备的干预效果估计方法，但其前提假设（如无未观测混杂因子、因果充分性等）在生态系统中往往难以满足。因此，在生态学因果推断中，最稳健的策略仍是将强理论（领域知识指导的SEM框架）、强数据（精心设计的观测或实验）和强方法（因果机器学习的辅助探索）三者结合，而非依赖单一途径。
...
**错误1：对计数数据使用线性回归。** 物种个体数、开花数、种子数是计数数据，使用普通线性回归（假设正态分布）会导致负值预测、异方差等问题。应使用泊松回归或负二项回归，并始终检查过度离散。

**错误2：忽略随机效应的结构。** 如果你的数据有分组结构（同一物种的多个个体、同一样方的多次调查），必须使用混合效应模型。将所有变量当作固定效应处理会导致伪重复和标准误的低估（假阳性增加）。

**错误3：GAM平滑参数完全交给默认值。** `gam()`函数的默认平滑参数不一定适合你的数据。过平滑会忽略重要的生态模式，欠平滑会引入噪声。检查`gam.check()`的输出，并根据生态学知识决定合理的平滑自由度。

**错误4：忽略空间自相关。** 生态数据几乎都有空间结构——邻近的样方更相似。忽略空间自相关会导致参数估计有效样本量被高估，标准误被低估，p值过于乐观。先用Moran's I检验，如果显著，使用空间回归模型或空间交叉验证。

## 本章要点回顾

**林小雨学到了什么**：当天童山的树木生长数据无法用简单线性模型描述时，她转向了GAM——让数据自己"说话"揭示光照与生长的非线性饱和关系。当需要同时建模不同树种的生长差异时，混合效应模型让她能够用一个统一的框架同时估计群体平均效应和物种个体偏差。当样地的空间位置开始"影响"结果时，空间自相关模型让她的推断重新变得可靠。

**统计-AI桥梁**：她理解了GAM的平滑样条本质上是用预设的基函数组合来拟合非线性关系，而神经网络用可学习的基函数做同样的事。GLM的logit link将线性预测映射到概率空间，与神经网络的sigmoid激活函数是同一数学操作。混合效应模型的分层结构与层次贝叶斯神经网络共享同一个统计逻辑。

**生态学意义**：现实生态系统充满非线性、多层次结构和空间依赖。高级回归模型让林小雨能够用更诚实的数学语言描述这些复杂性，而不是强行将复杂的生态关系塞入简单的线性框架中。
