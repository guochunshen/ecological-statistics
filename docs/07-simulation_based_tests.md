...
**常林的选择**。回到天童山的实际场景，常林正在分析群落数据以检验环境过滤假说。她的数据来自20个样方（每个20m×20m），记录了50种木本植物的多度。在这个数据规模下，她明智地选择了传统的固定行列总和零模型，因为20个样方远不足以训练任何有意义的深度生成模型。但她理解了自己选择的理论基础，知道如果未来有了更大规模的数据（如天童山样地扩展到500个样方，或者整合多个森林监测网络的数据），深度生成零模型将为她打开检验更精细生态假说的大门。这种知道自己方法的边界、知道何时需要升级工具的意识，正是AI时代我们最重要的专业素养。
