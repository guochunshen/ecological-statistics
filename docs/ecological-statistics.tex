...
\item
  \textbf{计算公式是：} P(蚱蜢选择黑麦草) = （有利于该事件的结果数）/ （所有可能的结果数） = 1/3。
\item
...
\item
  \textbf{度量方式为：} P(选择黑麦草) ≈ 45/70 ≈ 0.64； P(选择混合草甸) ≈ 20/70 ≈ 0.29； P(选择三叶草) ≈ 5/70 ≈ 0.07。
\item
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{客观性}：基于实际观察数据，避免了主观假设。
\item
  \textbf{可验证性}：结果可以通过重复试验来验证。
\item
  \textbf{实用性}：适用于各种现实世界的概率估计问题。
\item
  \textbf{渐进精确性}：随着样本量的增加，估计会越来越准确。
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{需要大量数据}：为了获得可靠的估计，通常需要大量的观察数据。
\item
  \textbf{无法处理一次性事件}：对于无法重复的事件（如特定物种的灭绝），频率概率难以应用。
\item
  \textbf{历史依赖性}：基于历史数据的概率估计可能无法反映未来的变化。
\item
  \textbf{样本偏差}：如果样本不具有代表性，频率估计会产生偏差。
\end{enumerate}
...
\subsubsection{动态的更新------贝叶斯概率}\label{ux52a8ux6001ux7684ux66f4ux65b0ux8d1dux53f6ux65afux6982ux7387}}
...
\item
  \textbf{它的思维是动态更新的：} 我原来的信念（P(选择三叶草)=0.07）是\textbf{先验概率}。得到``昨天下过雨''这个\textbf{证据}后，我利用一个公式（贝叶斯定理）将先验概率和证据结合起来，得到一个更新后的\textbf{后验概率}。
\item
  这个后验概率可能变成 P(选择三叶草 \textbar{} 昨天下过雨) = 0.25。这意味着，在``雨后''这个条件下，我认为你选择三叶草的概率从7\%显著提升到了25\%。贝叶斯概率让我们的认知能够随着新证据的出现而不断进化，更像是一种科学的学习过程。
\end{itemize}

\hypertarget{ux968fux673aux53d8ux91cfux4e0eux5206ux5e03}{%
\section{随机变量与分布}\label{ux968fux673aux53d8ux91cfux4e0eux5206ux5e03}}

\hypertarget{ux7b2cux56dbux5e55ux4eceux4e8bux4ef6ux5230ux53d8ux91cfux968fux673aux53d8ux91cfux4e0eux6982ux7387ux5206ux5e03}{%
\subsubsection{\texorpdfstring{\textbf{第四幕：从事件到变量------随机变量与概率分布}}{第四幕：从事件到变量------随机变量与概率分布}}\label{ux7b2cux56dbux5e55ux4eceux4e8bux4ef6ux5230ux53d8ux91cfux968fux673aux53d8ux91cfux4e0eux6982ux7387ux5206ux5e03}}
