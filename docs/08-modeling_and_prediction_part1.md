...
``` r
# 选择最优模型
best_degree <- which.min(c(AIC(degree_1), AIC(degree_2), AIC(degree_3), AIC(degree_4)))
cat("\n最优多项式阶数:", best_degree, "\n")
```
...
``` r
cat("线性模型 R²:", round(summary(linear_model)$r.squared, 4), "\n")
```
...
``` r
cat("二次多项式 R²:", round(summary(quadratic_model)$r.squared, 4), "\n")
```
...
``` r
cat("7次多项式 R²:", round(summary(overfit_model)$r.squared, 4), "\n")
```
...
``` r
cat("二次多项式 CV误差:", round(cv_quadratic, 4), "\n")
```
...
``` r
cat("7次多项式 CV误差:", round(cv_overfit, 4), "\n")
```
