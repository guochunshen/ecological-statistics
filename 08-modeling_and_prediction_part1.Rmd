...
overfit_model <- lm(y ~ poly(x, degree = 9), data = overfit_data)  # 9次多项式
```
...
cat("9次多项式 R²:", round(summary(overfit_model)$r.squared, 4), "\n")
...
cat("数据点数量:", n_points, "\n")
cat("多项式阶数:", 9, "\n")
cat("自由度:", n_points - 10, "（负值表明过度参数化）\n")

# 计算训练误差和泛化误差（通过交叉验证）
library(boot)

# 二次多项式的交叉验证误差
cv_quadratic <- cv.glm(overfit_data, quadratic_model, K = 5)$delta[1]
# 9次多项式的交叉验证误差
cv_overfit <- cv.glm(overfit_data, overfit_model, K = 5)$delta[1]

cat("\n=== 交叉验证误差比较 ===\n")
cat("二次多项式 CV误差:", round(cv_quadratic, 4), "\n")
cat("9次多项式 CV误差:", round(cv_overfit, 4), "\n")

if (cv_overfit > cv_quadratic) {
  cat("✓ 二次多项式具有更好的泛化能力\n")
} else {
  cat("⚠ 9次多项式可能过度拟合训练数据\n")
}
...
这个例子清楚地展示了过度拟合的本质：9次多项式（紫色曲线）几乎完美地通过了所有10个数据点，R²接近1，但它的预测行为在数据点之间变得极其不稳定，产生了不合理的波动。相比之下，二次多项式（绿色曲线）虽然R²较低，但更接近真实的生态关系（黑色虚线），具有更好的泛化能力。
