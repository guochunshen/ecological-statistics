...
# 结果输出：整体模型结果
knitr::kable(summary(overall_model)$coefficients, caption = "整体回归模型结果")
```
...
```{r, echo=FALSE}
# 结果输出：模型摘要
knitr::kable(summary(multi_model)$coefficients, caption = "多元线性回归模型结果")
```
...
knitr::kable(summary(full_model)$coefficients, caption = "完整模型结果")
```
...
# 多项式模型摘要
knitr::kable(summary(poly_model)$coefficients, caption = "多项式模型摘要")
