...
# 保存数据到data文件夹
save(forest_survey_data, file = "data/forest_survey_data.rda")
```
...
```{r forest-survey-plot, fig.cap="林小雨的森林调查数据：温度与植物生长速率的关系散点图及线性回归线。图中显示温度从10℃到30℃时，植物生长速率呈现明显的正相关关系，线性回归线（红色虚线）表明温度每升高1℃，生长速率平均增加约0.5单位。", echo=FALSE}
# 加载森林调查数据
load("data/forest_survey_data.rda")
...
# 保存数据到data文件夹
save(all_data, file = "data/r2_simulation_data.rda")
```

```{r, fig.cap="不同R²值（决定系数）的生态学含义可视化。左图（R²=0.1）显示环境因子对生态响应影响较弱；中图（R²=0.5）表明环境因子是重要驱动因素；右图（R²=0.9）显示环境因子是生态响应的主要决定因素，模型具有很强的预测能力。", echo=FALSE}
# 加载R²模拟数据
...
```{r r2-adj-r2-comparison, fig.cap="R²与调整R²随自变量数量变化的关系。蓝色实线显示R²随变量增加持续上升，即使添加无关变量；红色虚线显示调整R²在真实变量数量（2个）后开始下降，惩罚模型复杂度，避免过度拟合。", echo=FALSE}
...
# 添加调整R²值变化曲线
lines(1:10, adj_r2_values, type = "b", col = "red", lwd = 2, pch = 17, lty = 2)
...
```{r model-significance-comparison, fig.cap="整体模型显著性比较。左图显示显著模型（p < 0.05），环境因子对生态响应有显著影响；右图显示不显著模型（p > 0.05），没有证据表明环境因子有显著影响。", echo=FALSE}
# 演示整体模型显著性的概念
set.seed(789)
...
plot(y_sig ~ x_sig,
     ...
     xlab = "环境因子", ylab = "生态响应",
     pch = 16, col = "darkgreen")
abline(model_sig, col = "red", lwd = 2, lty = 1)
...
plot(y_nonsig ~ x_nonsig,
     ...
     xlab = "环境因子", ylab = "生态响应",
     pch = 17, col = "gray")
abline(model_nonsig, col = "red", lwd = 2, lty = 2)
...
```{r residual-vs-fitted-plot, fig.cap="残差 vs 拟合值图。检查线性性和同方差性假设，理想情况下残差应随机分布在水平线y=0周围，无明显的模式或趋势。"}
# 诊断图1：残差 vs 拟合值图
# 这个图主要用于检查线性性和同方差性假设
...
```{r qq-plot, fig.cap="正态Q-Q图。检查残差的正态性假设，理想情况下标准化残差应大致沿着45度对角线分布，无系统性偏离。"}
# 诊断图2：正态Q-Q图
# 这个图用于检查残差的正态性假设
...
```{r scale-location-plot, fig.cap="尺度-位置图。检查同方差性假设，展示标准化残差的平方根与拟合值的关系，理想情况下点应围绕水平线随机分布。"}
# 诊断图3：尺度-位置图
# 这个图提供了另一种检查同方差性的视角
...
```{r residual-leverage-plot, fig.cap="残差 vs 杠杆图。识别异常值和有影响的观测点，特别关注同时具有高杠杆和大残差的点，这些点可能对模型结果产生不成比例的影响。"}
# 诊断图4：残差 vs 杠杆图
# 这个图用于识别异常值和有影响的观测点
...
save(forest_data, file = "data/forest_data.rda")
```
...
save(forest_multi_data, file = "data/forest_multi_data.rda")
```
...
```{r simple-lm-full-model, echo=FALSE}
knitr::kable(summary(full_model)$coefficients, caption = "完整模型结果", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```
...
```{r simple-lm-step-model, echo=FALSE}
knitr::kable(summary(step_model)$coefficients, caption = "逐步回归模型结果", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```
...
```{r variable-importance-plot, fig.cap="变量相对重要性柱状图：使用LMG方法计算的各预测变量对模型解释的相对贡献度，数值越大表示变量在解释响应变量变异中的重要性越高", echo=FALSE}
# 4. 变量重要性分析
library(relaimpo)
importance_result <- calc.relimp(full_model, type = "lmg", rela = TRUE)

# 创建变量重要性柱状图
importance_df <- data.frame(
  Variable = names(importance_result$lmg),
  Importance = importance_result$lmg
)
...
save(forest_ecosystem_data, file = "data/forest_ecosystem_data.rda")
```
...
```{r, pairs-plot, fig.cap="物种丰富度与主要环境因子的散点图矩阵。展示栖息地面积、植被密度、距水源距离和土壤pH值与物种丰富度之间的两两关系，用于初步探索变量间的相关性和分布特征。"}
...
save(altitude_data, file="data/altitude_data.Rdata")
```

```{r, fig.cap="物种丰富度与海拔关系的多项式回归分析。蓝色圆点为观测数据，红色虚线为线性回归线，绿色实线为二次多项式回归曲线。多项式模型更好地捕捉了物种丰富度随海拔变化的单峰分布模式。"}
# 多项式回归示例：物种丰富度与海拔的关系
...
save(overfit_forest_data, file="data/overfit_forest_data.Rdata")
```
...
```{r, overfitting-demo, fig.cap="多项式回归的过度拟合问题演示。蓝色圆点为观测数据，黑色虚线为真实关系，红色实线为线性回归，绿色虚线为二次多项式，紫色点线为7次多项式。高次多项式过度拟合训练数据，产生不合理的波动，泛化能力差。", echo=FALSE}
# 生成预测曲线
...
# 添加拟合曲线
lines(altitude_seq, pred_linear, col = "red", lwd = 2, lty = 1)
lines(altitude_seq, pred_quadratic, col = "darkgreen", lwd = 2, lty = 2)
lines(altitude_seq, pred_overfit, col = "purple", lwd = 2, lty = 3)
...
# 模拟光合作用数据
set.seed(2024)
temperature <- seq(15, 35, length.out = 50)
# 真实关系：光合作用速率 = 5 + 0.8×温度 - 0.02×温度² + 随机误差
photosynthesis_rate <- 5 + 0.8 * temperature - 0.02 * temperature^2 + rnorm(50, 0, 1)

plant_data <- data.frame(
  temperature = temperature,
  photosynthesis = photosynthesis_rate
)
save(plant_data, file="data/plant_data_p1.Rdata")
```
...
set.seed(2024)
n_sites <- 60
habitat_area <- runif(n_sites, 10, 100)        # 栖息地面积(公顷)
vegetation_density <- runif(n_sites, 0.3, 0.9) # 植被密度
canopy_cover <- runif(n_sites, 0.4, 0.95)      # 林冠覆盖度
distance_to_edge <- runif(n_sites, 0.1, 2)     # 距林缘距离(km)
soil_moisture <- runif(n_sites, 0.2, 0.8)      # 土壤湿度

# 真实关系：鸟类丰富度主要受栖息地面积和植被密度影响
bird_richness <- rpois(n_sites,
                       exp(2.0 + 0.015 * habitat_area +
                             1.5 * vegetation_density +
                             rnorm(n_sites, 0, 0.3)))
...
bird_data <- data.frame(
  ...
  vegetation = vegetation_density,
  canopy = canopy_cover,
  edge_distance = distance_to_edge,
  soil_moisture = soil_moisture
)
save(bird_data, file="data/bird_data_p1.Rdata")
```
...
set.seed(2024)
n_lakes <- 45
nutrient_concentration <- runif(n_lakes, 5, 50)  # 营养盐浓度(μg/L)
water_temperature <- runif(n_lakes, 10, 25)      # 水温(°C)
light_intensity <- runif(n_lakes, 100, 500)      # 光照强度(μmol/m²/s)

# 真实关系：浮游植物生物量呈现饱和增长模式
# 使用Michaelis-Menten动力学模拟
max_biomass <- 100  # 最大生物量
half_saturation <- 20  # 半饱和常数

phytoplankton_biomass <- max_biomass * nutrient_concentration /
  (half_saturation + nutrient_concentration) +
  rnorm(n_lakes, 0, 8)

lake_data <- data.frame(
  biomass = phytoplankton_biomass,
  nutrient = nutrient_concentration,
  temperature = water_temperature,
  light = light_intensity
)
save(lake_data, file="data/lake_data_p1.Rdata")
```
